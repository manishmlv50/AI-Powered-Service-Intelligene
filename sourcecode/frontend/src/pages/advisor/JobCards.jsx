...
export default function JobCards() {
    ...
    const fetchJobs = useCallback(async (status) => {
        try {
            ...
            const res = await listJobCards(params, {
            ...
            if (err.name !== 'AbortError') {
                console.error('JobCards fetch error:', err)
            }
        } finally {
            setLoading(false)
        }
                    ...
                    <table className="data-table">
                        <thead>
                            <tr>
                                ...
                                <th>Complaint</th><th>Service Type</th>
                        ...
                        <tbody>
                            {jobs.map((j,i)=>(
                                ...
                                >
                                    ...
                                    <td className="truncate" style={{ color:'var(--text-muted)', fontSize:'0.85rem', maxWidth:200 }}>
                                        {j.complaint}
                                    </td>
                                    ...
                                    <td>
                                        <div style={{ display:'flex', gap:4 }}>
                                            <button
                                                ...
                                                onClick={e=>{ e.stopPropagation(); navigate(`/advisor/jobs/${j.id}`)}}
                                            >
                                                <Eye size={13}/>
                                            </button>
                                            <button
                                                className="btn btn-outline btn-sm"
                                                onClick={e=>{ e.stopPropagation(); navigate(`/advisor/jobs/${j.id}?edit=1`)}}
                                            >
                                                <Edit size={13}/>
                                            </button>
                                        </div>
